...
<div class="comic-card" id="comic-card-<%=comic.getISBN()%>">
    ...
    <button class="wish-container" data-is-wished="<%= isWished %>" onclick="toggleWish('<%=comic.getISBN()%>')" id="wish-container-<%=comic.getISBN()%>" aria-label="Toggle Wish" onkeydown="toggleWish(<%=comic.getISBN()%>)">
    ...
    <button class="button-product" onclick='addCart("<%=comic.getISBN()%>", 1, <%=comicJson%>)' aria-label="Aggiungi <%=comic.getTitle()%> al carrello">AGGIUNGI AL CARRELLO</button>
</div>